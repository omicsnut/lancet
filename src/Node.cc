...
void Node_t::addEdge(Mer_t nodeid, Edgedir_t dir, ReadId_t readid) {
  ...
  for (unsigned int i = 0; i < edges_m.size(); ++i) {
    if ((edges_m[i].nodeid_m == nodeid) && (edges_m[i].dir_m == dir)) {
      if (readid != -1) {
...
void Node_t::updateEdge(const Mer_t& oldid, Edgedir_t olddir,
  ...
  bool found = false;
  for (unsigned int i = 0; i < edges_m.size(); ++i) {
    if (edges_m[i].nodeid_m == oldid && edges_m[i].dir_m == olddir) {
  ...
  if (!found) {
    cerr << "ERROR updating " << nodeid_m
         << ": didn't find: " << Edge_t::toString(olddir) << ":" << oldid
         << endl;
    cerr << "wanted to replace with " << Edge_t::toString(newdir) << ":"
         << newid << endl;
    print(cerr);
  }
...
void Node_t::removeEdge(const Mer_t& nodeid, Edgedir_t dir) {
  ...
  if (!found) {
    cerr << "ERROR removing " << nodeid_m << ": didn't find: " << dir << ":"
         << nodeid << endl;
  }
...
// getBuddy: If node has a single edge in the dir direction, return edge id,
// else -1
//////////////////////////////////////////////////////////////
...
void Node_t::addReadStart(ReadId_t readid, int nodeoffset, int trim5,
                          Ori_t ori) {
...
int Node_t::cntReadCode(char code) {
  ...
  for (auto si = reads_m.begin(); si != reads_m.end(); ++si) {
    if ((*si) < 0 || (*si) > (int)readid2info->size()) {
      continue;
    }  // skip over illegal values
...
//////////////////////////////////////////////////////////////
void Node_t::updateCovDistr(int c, const string& qv, unsigned int strand,
                            char sample) {
...
void Node_t::computeMinCov() {
  ...
  for (unsigned int i = 0; i < cov_distr_tmr.size(); ++i) {
    int totcov = cov_distr_tmr[i].fwd + cov_distr_tmr[i].rev +
                 cov_distr_nml[i].fwd + cov_distr_nml[i].rev;
    int totcovQV = cov_distr_tmr[i].minqv_fwd + cov_distr_tmr[i].minqv_rev +
                   cov_distr_nml[i].minqv_fwd + cov_distr_nml[i].minqv_rev;
...
bool Node_t::hasOverlappingMate(string& read_name, int id) {
  ...
  if (id == 1) {
    ...
    // for (vector<string>::iterator it2 = mate2_name.begin() ; it2 !=
    // mate2_name.end(); ++it2) { 	if ((*it2) == read_name) { ans = true; }
    //}
  }
  ...
  if (id == 2) {
    ...
    // for (vector<string>::iterator it1 = mate1_name.begin() ; it1 !=
    // mate1_name.end(); ++it1) { 	if ((*it1) == read_name) { ans = true; }
    //}
  }
...
void Node_t::addMateName(string& read_name, int id) {
  ...
  if (id == 1) {
    mate1_name.push_back(read_name);
  }
  if (id == 2) {
    mate2_name.push_back(read_name);
  }
}
