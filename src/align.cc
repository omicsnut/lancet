...
cell mk_cell(int s, char t) { return cell(s, t); }
...
cell maxscorexy(cell scorex, cell scorey, int scorez) {
  ...
  if (scorex.score > r.score) {
    r.score = scorex.score;
    r.tb = '<';
  }
  if (scorey.score > r.score) {
    r.score = scorey.score;
    r.tb = '^';
  }
...
void global_align(const string& S, const string& T, string& S_aln,
                  string& T_aln, int endfree, int V) {
  S_aln.clear();
  T_aln.clear();
  ...
  if (endfree) {
    ...
    if (V) {
      cout << "Tracking back from: " << i << " score: " << maxval << endl;
    }
  }
...
void global_align_aff(const string& S, const string& T, string& S_aln,
                      string& T_aln, int endfree, int V) {
  S_aln.clear();
  T_aln.clear();
  ...
  for (j = 0; j <= m; ++j) {
    X[0][j] = mk_cell(GAP_OPEN + j * GAP_EXTEND, '^');
    M[0][j] = X[0][j];
  }
  for (i = 0; i <= n; ++i) {
    Y[i][0] = mk_cell(GAP_OPEN + i * GAP_EXTEND, '<');
    M[i][0] = Y[i][0];
  }
  M[0][0] = mk_cell(0, '*');
  ...
  for (int j = 1; j <= m; ++j) {
    ...
    for (int i = 1; i <= n; ++i) {
      X[i][j] =
          maxx(X[i - 1][j].score + GAP_EXTEND, M[i - 1][j].score + GAP_OPEN);

      Y[i][j] =
          maxy(Y[i][j - 1].score + GAP_EXTEND, M[i][j - 1].score + GAP_OPEN);

      M[i][j] = maxscorexy(X[i][j], Y[i][j],
                           M[i - 1][j - 1].score + cmp(S[i - 1], T[j - 1]));

      if (V) {
        printf(" % 3d%c%c", M[i][j].score, M[i][j].tb, (i == j) ? '*' : ' ');
      }
    }
  ...
  if (V) {
    cout << "Score is " << M[n][m].score << endl;
  }
...
void global_cov_align_aff(const string& S, const string& T,
                          const vector<int>& CT, string& S_aln, string& T_aln,
                          vector<int>& CT_aln, int endfree, int V) {
  S_aln.clear();
  T_aln.clear();
  CT_aln.clear();
