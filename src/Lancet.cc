...
void printUsage() {
  ...
  string USAGE =
      "\nUsage: lancet [options] --tumor <BAM file> --normal <BAM file> --ref "
      "<FASTA file> --reg <chr:start-end>\n [-h for full list of commands]\n\n";
...
void printHelpText(Filters &filters) {
  ...
  helptext
      << "Required\n"
         "   --tumor, -t              <BAM file>    : BAM file of mapped reads "
         "for tumor\n"
         "   --normal, -n             <BAM file>    : BAM file of mapped reads "
         "for normal\n"
         "   --ref, -r                <FASTA file>  : FASTA file of reference "
         "genome\n"
         "   --reg, -p                <string>      : genomic region (in "
         "chr:start-end format)\n"
         "   --bed, -B                <string>      : genomic regions from "
         "file (BED format)\n"

         "\nOptional\n"
         "   --min-k, k                <int>         : min kmersize [default: "
      << minK
      << "]\n"
         "   --max-k, -K               <int>         : max kmersize [default: "
      << maxK
      << "]\n"
         "   --trim-lowqual, -q        <int>         : trim bases below qv at "
         "5' and 3' [default: "
      << MIN_QV_TRIM
      << "]\n"
         "   --min-base-qual, -C       <int>         : minimum base quality "
         "required to consider a base for SNV calling [default: "
      << MIN_QV_CALL
      << "]\n"
         "   --quality-range, -Q       <char>        : quality value range "
         "[default: "
      << (char)QV_RANGE
      << "]\n"
         "   --min-map-qual, -b        <int>         : minimum read mapping "
         "quality in Phred-scale [default: "
      << MIN_MAP_QUAL
      << "]\n"
         "   --max-as-xs-diff, -Z      <int>         : maximum different "
         "between AS and XS alignments scores [default: "
      << MAX_DELTA_AS_XS
      << "]\n"
         "   --tip-len, -l             <int>         : max tip length "
         "[default: "
      << MAX_TIP_LEN
      << "]\n"
         "   --cov-thr, -c             <int>         : min coverage threshold "
         "used to select reference anchors from the De Bruijn graph [default: "
      << COV_THRESHOLD
      << "]\n"
         "   --cov-ratio, -x           <float>       : minimum coverage ratio "
         "used to remove nodes from the De Bruijn graph [default: "
      << MIN_COV_RATIO
      << "]\n"
         "   --low-cov, -d             <int>         : low coverage threshold "
         "used to remove nodes from the De Bruijn graph [default: "
      << LOW_COV_THRESHOLD
      << "]\n"
         "   --max-avg-cov, -u         <int>         : maximum average "
         "coverage allowed per region [default: "
      << MAX_AVG_COV
      << "]\n"
         "   --window-size, -w         <int>         : window size of the "
         "region to assemble (in base-pairs) [default: "
      << WINDOW_SIZE
      << "]\n"
         "   --padding, -P             <int>         : left/right padding (in "
         "base-pairs) applied to the input genomic regions [default: "
      << PADDING
      << "]\n"
         "   --dfs-limit, -F           <int>         : limit dfs/bfs graph "
         "traversal search space [default: "
      << DFS_LIMIT
      << "]\n"
         "   --max-indel-len, -T       <int>         : limit on size of "
         "detectable indel [default: "
      << MAX_INDEL_LEN
      << "]\n"
         "   --max-mismatch, -M        <int>         : max number of "
         "mismatches for near-perfect repeats [default: "
      << MAX_MISMATCH
      << "]\n"
         "   --num-threads, -X         <int>         : number of parallel "
         "threads [default: "
      << NUM_THREADS
      << "]\n"
         //		"   --rg-file, -g             <string>      : read group
         // file\n"
         "   --node-str-len, -L        <int>         : length of sequence to "
         "display at graph node (default: "
      << NODE_STRLEN
      << ")\n"

         "\nFilters\n"
         "   --min-alt-count-tumor, -a  <int>        : minimum alternative "
         "count in the tumor [default: "
      << filters.minAltCntTumor
      << "]\n"
         "   --max-alt-count-normal, -m <int>        : maximum alternative "
         "count in the normal [default: "
      << filters.maxAltCntNormal
      << "]\n"
         "   --min-vaf-tumor, -e        <float>      : minimum variant allele "
         "frequency (AlleleCov/TotCov) in the tumor [default: "
      << filters.minVafTumor
      << "]\n"
         "   --max-vaf-normal, -i       <float>      : maximum variant allele "
         "frequency (AlleleCov/TotCov) in the normal [default: "
      << filters.maxVafNormal
      << "]\n"
         "   --min-coverage-tumor, -o   <int>        : minimum coverage in the "
         "tumor [default: "
      << filters.minCovTumor
      << "]\n"
         "   --max-coverage-tumor, -y   <int>        : maximum coverage in the "
         "tumor [default: "
      << filters.maxCovTumor
      << "]\n"
         "   --min-coverage-normal, -z  <int>        : minimum coverage in the "
         "normal [default: "
      << filters.minCovNormal
      << "]\n"
         "   --max-coverage-normal, -j  <int>        : maximum coverage in the "
         "normal [default: "
      << filters.maxCovNormal
      << "]\n"
         "   --min-phred-fisher, -s     <float>      : minimum fisher exact "
         "test score [default: "
      << filters.minPhredFisher
      << "]\n"
         "   --min-phred-fisher-str, -E <float>      : minimum fisher exact "
         "test score for STR mutations [default: "
      << filters.minPhredFisherSTR
      << "]\n"
         "   --min-strand-bias, -f      <float>      : minimum strand bias "
         "threshold [default: "
      << filters.minStrandBias
      << "]\n"

         "\nShort Tandem Repeat parameters\n"
         "   --max-unit-length, -U      <int>        : maximum unit length of "
         "the motif [default: "
      << MAX_UNIT_LEN
      << "]\n"
         "   --min-report-unit, -N      <int>        : minimum number of units "
         "to report [default: "
      << MIN_REPORT_UNITS
      << "]\n"
         "   --min-report-len, -Y       <int>        : minimum length of "
         "tandem in base pairs [default: "
      << MIN_REPORT_LEN
      << "]\n"
         "   --dist-from-str, -D        <int>        : distance (in bp) of "
         "variant from STR locus [default: "
      << DIST_FROM_STR
      << "]\n"

         "\nFlags\n"
         "   --primary-alignment-only, -I  : only use primary alignments for "
         "variant calling\n"
         "   --XA-tag-filter, -O           : skip reads with multiple hits "
         "listed in the XA tag (BWA only)\n"
         "   --active-region-off, -W       : turn off active region module\n"
         "   --kmer-recovery, -R           : turn on k-mer recovery "
         "(experimental)\n"
         "   --print-graph, -A             : print graph (in .dot format) "
         "after every stage\n"
...
void printConfiguration(ostream &out, Filters &filters) {
  ...
  // out << "INSERT_SIZE: "      << INSERT_SIZE << " +/- " << INSERT_STDEV <<
  // endl;
...
int loadRefs(const string reference, const string region,
  ...
  if ((x == string::npos) &&
      (hdr.length() >
       0)) {  // no ":" symbol found -> assume single chromosome name format
    CHR = hdr.substr(0, x);
    START = "1";
    ...
    if (it == bamrefs.end()) {
      cerr << "ERROR: chromosome label " << CHR << " not found in BAM header!"
           << endl;
    }
    ...
    if (SP < 1) {
      SP = 1;
    }  // start position cannnot be less than 1
  ...
  if (!fai) {
    cerr << "Could not load fai index of " << reference << endl;
  }
  ...
  if (seq_len < 0) {
    cerr << "Failed to fetch sequence in " << REG << endl;
  }
...
int rLancet(string tumor_bam, string normal_bam, string ref_fasta, string reg,
  ...
  if (TUMOR == "") {
    cerr << "ERROR: Must provide the tumor BAM file (-t)" << endl;
    errflg = true;
  }
  if (NORMAL == "") {
    cerr << "ERROR: Must provide the normal BAM file (-n)" << endl;
    errflg = true;
  }
  if (REFFILE == "") {
    cerr << "ERROR: Must provide a reference genome file (-r)" << endl;
    errflg = true;
  }
  if ((BEDFILE == "") && (REGION == "")) {
    cerr << "ERROR: Must provide region (-p) or BED file (-B)" << endl;
    errflg = true;
  }
  ...
  if (verbose) {
    printConfiguration(cerr, filters);
  }
  ...
  if (!found && ACTIVE_REGIONS) {
    ...
    cerr << "MD tag required to select active regions, but is missing from "
            "alignments."
         << endl;
    cerr << "RECOMMENDED ACTION: turn off the active region module "
            "(--active-region-off)"
         << endl;
    cerr << "-----------------------" << endl << endl;
  }
  ...
  RefVector references =
      readerT.GetReferenceData();  // Extract all reference sequence entries.
  ...
  try {
    ...
    vector<map<string, Ref_t *> > reftables(
        NUM_THREADS, map<string, Ref_t *>());  // table of references to analyze
    ...
    for (i = 0; i < NUM_THREADS; ++i) {
      cerr << "starting thread " << (i + 1) << " on " << reftables[i].size()
           << " windows" << endl;
    ...
    for (i = 0; i < NUM_THREADS; ++i) {
      ...
      tot_snv_or_indel_or_softclip +=
          assemblers[i]->num_snv_or_indel_or_softclip_regions;
    ...
    // if(verbose) {
    cerr << "Total # of skipped windows: " << tot_skip << " ("
         << (100 * (double)tot_skip / double(num_windows)) << "\%)" << endl;
    ...
    cerr << "- # of windows with indels or softclips: " << tot_indel_or_softclip
         << endl;
    cerr << "- # of windows with SNVs or indels: " << tot_snv_or_indel << endl;
    cerr << "- # of windows with SNVs or softclips: " << tot_snv_or_softclip
         << endl;
    cerr << "- # of windows with SNVs or indels or softclips: "
         << tot_snv_or_indel_or_softclip << endl;
    //}
    ...
    variantDB.printToVCF(VERSION, REFFILE, DATE, filters,
                         assemblers[0]->sample_name_normal,
                         assemblers[0]->sample_name_tumor);
  } catch (int e) {
...
int main(int argc, char **argv) {
  ...
  // "u:m:n:r:g:s:k:K:l:t:c:d:x:BDRACIhSL:T:M:vF:q:b:Q:P:p:E")) != EOF))
  while (!errflg &&
         ((ch = getopt_long(argc, argv,
                            "u:n:r:g:k:K:l:f:t:c:C:d:x:ARhHSIWOL:T:P:M:vVF:q:b:"
                            "B:Q:p:s:E:a:m:e:i:o:y:z:w:j:X:U:N:Y:D:Z:",
    ...
    switch (ch) {
      ...
      case 'K':
        ...
        break;
      case 'l':
        MAX_TIP_LEN = atoi(optarg);
        break;
      // case 't': MIN_THREAD_READS = atoi(optarg); break;
      case 'c':
        COV_THRESHOLD = atoi(optarg);
        break;
      case 'x':
        MIN_COV_RATIO = atof(optarg);
        break;
      case 'd':
        LOW_COV_THRESHOLD = atoi(optarg);
        break;
      case 'w':
      ...
      case 'P':
        ...
        break;
      case 'u':
        MAX_AVG_COV = atoi(optarg);
        break;

      case 'q':
        MIN_QV_TRIM = atoi(optarg);
        break;
      case 'C':
        MIN_QV_CALL = atoi(optarg);
        break;
      case 'b':
        MIN_MAP_QUAL = atoi(optarg);
        break;
      case 'Z':
        MAX_DELTA_AS_XS = atoi(optarg);
        break;
      case 'Q':
        QV_RANGE = *optarg;
        break;
      ...
      case 'L':
        NODE_STRLEN = atoi(optarg);
        break;
      case 'F':
        DFS_LIMIT = atoi(optarg);
        break;
      case 'X':
        NUM_THREADS = atoi(optarg);
        break;
      case 'T':
        MAX_INDEL_LEN = atoi(optarg);
        break;
      case 'M':
      ...
      case 'U':
        MAX_UNIT_LEN = atoi(optarg);
        break;
      case 'N':
        MIN_REPORT_UNITS = atoi(optarg);
        break;
      case 'Y':
        MIN_REPORT_LEN = atoi(optarg);
        break;
      case 'D':
        DIST_FROM_STR = atoi(optarg);
        break;

      case 'E':
        filters.minPhredFisherSTR = atof(optarg);
        break;
      case 's':
        filters.minPhredFisher = atof(optarg);
        break;
      case 'f':
        filters.minStrandBias = atof(optarg);
        break;
      case 'a':
        filters.minAltCntTumor = atoi(optarg);
        break;
      case 'm':
        filters.maxAltCntNormal = atoi(optarg);
        break;
      case 'e':
        filters.minVafTumor = atof(optarg);
        break;
      case 'i':
        filters.maxVafNormal = atof(optarg);
        break;
      case 'o':
        filters.minCovTumor = atoi(optarg);
        break;
      case 'y':
        filters.maxCovTumor = atoi(optarg);
        break;
      case 'z':
        filters.minCovNormal = atoi(optarg);
        break;
      case 'j':
        filters.maxCovNormal = atoi(optarg);
        break;

      case 'I':
        PRIMARY_ALIGNMENT_ONLY = 1;
        break;
      case 'O':
        XA_FILTER = 1;
        break;
  ...
  if (!found && ACTIVE_REGIONS) {
    cerr << endl << "--------WARNING--------" << endl;
    cerr << "The MD tag is required to select the active regions, but is "
            "missing from the alignments in the BAM(s) file(s)."
         << endl;
    cerr << "To avoid unpredictable behavior, the active region module has "
            "been automatically turned off (--active-region-off)"
         << endl;
    cerr << "-----------------------" << endl << endl;
  ...
  if (PRINT_CONFIG) {
    params_file.open("config.txt");
    printConfiguration(params_file,
                       filters);  // save parameters setting to file
  }
